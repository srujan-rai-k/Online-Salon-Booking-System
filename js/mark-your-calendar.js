...
(function($) {
    ...
    $.fn.markyourcalendar = function(opts) {
        ...
        this.getNavControl = function() {
            var previousWeekHtml = `<div id="myc-prev-week-container1">` + settings.prevHtml + `</div>`;
            var nextWeekHtml = `<div id="myc-prev-week-container1">` + settings.nextHtml + `</div>`;
            var monthYearHtml = `
                <div id="myc-current-month-year-container1">
                    ` + this.getMonthName(settings.startDate.getMonth()) + ' ' + settings.startDate.getFullYear() + `
                </div>
            `;

            var navHtml = `
                <div id="myc-nav-container1">
                    ` + previousWeekHtml + `
                    ` + monthYearHtml + `
                    ` + nextWeekHtml + `
        ...
        this.getDatesHeader = function() {
            ...
            var ret = `<div id="myc-dates-container1">` + tmp + `</div>`;
            return ret;
        }
        ...
        this.getAvailableTimes = function() {
            ...
            for (i = 0; i < 2; i++) {
                ...
                tmp += `
                    <div class="myc-day-time-container1" id="myc-day-time-container1-` + i + `">
                        ` + tmpAvailTimes + `
                        <div style="clear:both;"></div>
                    </div>
                `;
        ...
        this.on('click', '#myc-prev-week', function() {
            settings.startDate = settings.startDate.addDays(-2);
        ...
        var render = function() {
            ret = `
                <div id="myc-container1">
                    <div id="myc-nav-container1">` + instance.getNavControl() + `</div>
                    <div id="myc-week-container1">
                        <div id="myc-dates-container1">` + instance.getDatesHeader() + `</div>
                        <div id="myc-available-time-container1">` + instance.getAvailableTimes() + `</div>
                    </div>
                </div>
            `;
            instance.html(ret);
